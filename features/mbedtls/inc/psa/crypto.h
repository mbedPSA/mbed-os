...
typedef enum {
    ...
    PSA_CRYPTO_SUCCESS = 0,
    /** The requested operation or a parameter is not supported
        by this implementation. */
    PSA_CRYPTO_ERROR_NOT_SUPPORTED,
    /** The requested action is denied by a policy. */
    PSA_CRYPTO_ERROR_NOT_PERMITTED,
    /** An output buffer is too small. */
    PSA_CRYPTO_ERROR_BUFFER_TOO_SMALL,
    /** A slot is occupied, but must be empty to carry out the
        requested action. */
    PSA_CRYPTO_ERROR_OCCUPIED_SLOT,
    /** A slot is empty, but must be occupied to carry out the
        requested action. */
    PSA_CRYPTO_ERROR_EMPTY_SLOT,
    /** The requested action cannot be performed in the current state. */
    PSA_CRYPTO_ERROR_BAD_STATE,
    /** The parameters passed to the function are invalid. */
    PSA_CRYPTO_ERROR_INVALID_ARGUMENT,
    /** There is not enough runtime memory. */
    PSA_CRYPTO_ERROR_INSUFFICIENT_MEMORY,
    /** There is not enough persistent storage. */
    PSA_CRYPTO_ERROR_INSUFFICIENT_STORAGE,
    /** There was a communication failure inside the implementation. */
    PSA_CRYPTO_ERROR_COMMUNICATION_FAILURE,
    /** There was a storage failure that may have led to data loss. */
    PSA_CRYPTO_ERROR_STORAGE_FAILURE,
    /** A hardware failure was detected. */
    PSA_CRYPTO_ERROR_HARDWARE_FAILURE,
    /** A tampering attempt was detected. */
    PSA_CRYPTO_ERROR_TAMPERING_DETECTED,
    /** There is not enough entropy to generate random data needed
        for the requested action. */
    PSA_CRYPTO_ERROR_INSUFFICIENT_ENTROPY,
    /** The signature, MAC or hash is incorrect. */
    PSA_CRYPTO_ERROR_INVALID_SIGNATURE,
    /** The decrypted padding is incorrect. */
    PSA_CRYPTO_ERROR_INVALID_PADDING,
    /** An error occurred that does not correspond to any defined
        failure cause. */
    PSA_CRYPTO_ERROR_UNKNOWN_ERROR,
} psa_status_t;
