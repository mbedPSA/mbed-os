...
static int32_t call_secure_time_set_trusted_init(psa_msg_t *msg)
{
    ...
    if (msg->out_size[0] != sizeof(set_trusted_init_status) || msg->out_size[1] != sizeof(nonce)) {
        return PSA_INVALID_PARAMETERS;
    }

    set_trusted_init_status = secure_time_set_trusted_init_impl(&nonce);
    psa_write(msg->handle, 0, &set_trusted_init_status, sizeof(set_trusted_init_status));

    if (set_trusted_init_status == SECURE_TIME_SUCCESS) {
        psa_write(msg->handle, 1, &nonce, sizeof(nonce));
    }
...
static int32_t call_secure_time_set_trusted_commit(psa_msg_t *msg)
{
    ...
    if (!blob_size || msg->out_size[0] != sizeof(time_set_commit_status)) {
        return PSA_INVALID_PARAMETERS;
    }
    ...
    psa_write(msg->handle, 0, &time_set_commit_status, sizeof(time_set_commit_status));
...
static int32_t call_secure_time_set(psa_msg_t *msg)
{
    ...
    if (msg->in_size[0] != sizeof(new_time) || msg->out_size[0] != sizeof(time_set_status)) {
        return PSA_INVALID_PARAMETERS;
    }

    if (psa_read(msg->handle, 0, &new_time, sizeof(new_time)) != sizeof(new_time)) {
        SPM_PANIC("Failed to read the requested new time to set!\n");
    }
    time_set_status = secure_time_set_impl(new_time);
    psa_write(msg->handle, 0, &time_set_status, sizeof(time_set_status));
...
static int32_t call_secure_time_get(psa_msg_t *msg)
{
    ...
    if (msg->out_size[0] != sizeof(current_time)) {
        return PSA_INVALID_PARAMETERS;
    }

    current_time = secure_time_get_impl();

    psa_write(msg->handle, 0, &current_time, sizeof(current_time));
...
static int32_t call_secure_time_set_stored_public_key(psa_msg_t *msg)
{
    ...
    if (!key_size || msg->out_size[0] != sizeof(set_key_status)) {
        return PSA_INVALID_PARAMETERS;
    }
    ...
    psa_write(msg->handle, 0, &set_key_status, sizeof(set_key_status));
...
static int32_t call_secure_time_get_stored_public_key_size(psa_msg_t *msg)
{
    ...
    if (msg->out_size[0] != sizeof(get_key_size_status) || msg->out_size[1] != sizeof(actual_size)) {
        return PSA_INVALID_PARAMETERS;
    }

    get_key_size_status = secure_time_get_stored_public_key_size_impl(&actual_size);
    psa_write(msg->handle, 0, &get_key_size_status, sizeof(get_key_size_status));
...
static int32_t call_secure_time_get_stored_public_key(psa_msg_t *msg)
{
    ...
    if (msg->in_size[0] != sizeof(buf_size) || msg->out_size[0] != sizeof(get_key_status) || msg->out_size[2] != sizeof(actual_size)) {
        return PSA_INVALID_PARAMETERS;
    }

    if (psa_read(msg->handle, 0, &buf_size, sizeof(buf_size)) != sizeof(buf_size)) {
    ...
    psa_write(msg->handle, 0, &get_key_status, sizeof(get_key_status));
    psa_write(msg->handle, 2, &actual_size, sizeof(actual_size));
...
void secure_time_main(void *ptr)
{
    ...
    while (1) {
        ...
        if (signals == 0 || (signals | SECURE_TIME_WAIT_ANY_SID_MSK) != SECURE_TIME_WAIT_ANY_SID_MSK) {
